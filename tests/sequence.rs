...
fn push_increases_len() {
    ...
    assert_eq!(seq.len(), 0);
    let tokens = model.tokenize("hi", false, false).unwrap();
    seq.push(tokens[0]);
    assert_eq!(seq.len(), 1);
}
...
fn tokens_accessor_matches_push_order() {
    ...
    let ctx = Context::new(&model, &params).unwrap();
    let mut seq = ctx.sequence().unwrap();
    let tokens = model.tokenize("abc", false, false).unwrap();
    seq.extend(&tokens);
...
fn pop_decreases_len() {
    ...
    let ctx = Context::new(&model, &params).unwrap();
    let mut seq = ctx.sequence().unwrap();
    let tokens = model.tokenize("hello", false, false).unwrap();
    seq.extend(&tokens);
...
fn multiple_sequences_independent() {
    ...
    let tokens1 = model.tokenize("hello", false, false).unwrap();
    let tokens2 = model.tokenize("world", false, false).unwrap();
    seq1.extend(&tokens1);
    seq2.extend(&tokens2);
...
fn sequence_sample_matches_argmax() {
    ...
    let ctx = Context::new(&model, &params).unwrap();
    let mut seq = ctx.sequence().unwrap();
    let tokens = model.tokenize("once upon", false, false).unwrap();
    seq.extend(&tokens);
...
fn sequence_sample_does_not_require_mut() {
    ...
    let ctx = Context::new(&model, &params).unwrap();
    let mut seq = ctx.sequence().unwrap();
    let tokens = model.tokenize("test", false, false).unwrap();
    seq.extend(&tokens);
