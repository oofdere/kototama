...
impl Sequence {
    ...
    pub fn decode(&mut self) {
        if let Some(&last_token) = self.tokens.last() {
            ...
            // The last position is still in the KV cache from the original
            // push — llama_decode rejects duplicate positions with
            // InvalidInput, so clear the slot before re-decoding.
            self.ctx
                .actor()
