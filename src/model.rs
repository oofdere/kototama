...
impl Model {
    ...
    pub fn tokenize(&self, text: &str, add_special: bool, parse_special: bool) -> Vec<llama_token> {
        // Optimize: a token generally represents at least 1 byte (plus maybe a few special tokens).
        // By pre-allocating an upper bound, we can usually avoid calling llama_tokenize twice.
        let estimated_len = text.len() + if add_special { 4 } else { 0 };
        let mut tokens = vec![0i32; estimated_len.max(4)];
        ...
        // If it was negative, our estimate was too small. The negative value is -required_len.
        if n_tokens < 0 {
