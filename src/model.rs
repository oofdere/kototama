...
impl Model {
    ...
    pub fn tokenize(&self, text: &str, add_special: bool, parse_special: bool) -> Vec<llama_token> {
        // Optimize: guess the number of tokens initially to avoid two passes of `llama_tokenize` in most cases.
        // A common heuristic is 1 token per 3-4 bytes of text. We add some buffer + extra space for special tokens.
        let estimated_len = text.len() / 3 + 8;
        let mut tokens = vec![0i32; estimated_len];
        ...
        // If the initial buffer was too small, `llama_tokenize` returns negative value
        // indicating the required size (negative of required size).
        if n_tokens < 0 {
            let required_len = -n_tokens as usize;
            tokens.resize(required_len, 0);
