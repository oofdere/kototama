...
impl Model {
    ...
    pub fn tokenize(&self, text: &str, add_special: bool, parse_special: bool) -> Vec<llama_token> {
        ...
        // OPTIMIZATION: Use uninitialized memory (Vec::with_capacity + set_len) instead of zero-initialization (vec![0; len])
        // to avoid redundant memory writes before the C FFI call overwrites the buffer.
        ...
        let n_tokens = unsafe {
            llama_sys::llama_tokenize(
                ...
                text.len() as i32,
                tokens.as_mut_ptr(),
                tokens.capacity() as i32, // is this needed it's a vec
                add_special,
                parse_special,
            )
        };
