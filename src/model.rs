...
impl Model {
    ...
    pub fn chat_template(&self, name: Option<&str>) -> Option<String> {
        let name_cstr = name.map(|s| std::ffi::CString::new(s).ok()).flatten();
    ...
    pub fn tokenize(&self, text: &str, add_special: bool, parse_special: bool) -> Vec<llama_token> {
        let text_len = if text.len() > i32::MAX as usize {
            // Text length exceeds i32::MAX, which would overflow and be interpreted
            // as negative by the underlying C function, leading to buffer over-read.
